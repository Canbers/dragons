...
- For each NPC, give them a PERSONAL GOAL and a PROBLEM. These should be specific to this person and this place — not generic RPG quests. A barkeep's goal might be "keep the tavern afloat after her husband died" and her problem might be "the local gang wants protection money."
- For each NPC, include a "personality" (2-3 adjective personality like "blunt, impatient, secretly generous") and a "profession" (their social role like "blacksmith" or "retired soldier").

Also generate 1-2 TENSIONS — active conflicts or situations at this location. Involve the NPCs you created. Severity: "simmering" (background), "active" (noticeable), "boiling" (about to explode).
...
Return ONLY valid JSON matching this structure (do NOT copy these example names — generate unique ones for this specific location):
{
    ...
    "pois": [
        { "name": "<UNIQUE_NPC_NAME>", "type": "npc", "description": "<what they look like and are doing>", "disposition": "<personality and mood>", "personality": "<2-3 adjective personality>", "profession": "<their role>", "goal": "<what they want (one sentence)>", "problem": "<what blocks them (one sentence)>", "icon": "👤" },
        { "name": "<NOTABLE_OBJECT>", "type": "object", "description": "<brief description>", "icon": "📋" }
    ],
    "tensions": [
        { "description": "<active conflict or situation>", "involvedNpcs": ["<NPC Name>"], "severity": "simmering" }
    ]
}`;
    ...
    try {
        ...
        // Save tensions to location
        const validSeverities = ['simmering', 'active', 'boiling'];
        if (parsed.tensions && Array.isArray(parsed.tensions)) {
            location.tensions = parsed.tensions.slice(0, 2).map(t => ({
                description: t.description || '',
                involvedNpcs: Array.isArray(t.involvedNpcs) ? t.involvedNpcs : [],
                severity: validSeverities.includes(t.severity) ? t.severity : 'simmering'
            }));
            await settlement.save();
            console.log(`[FirstImpression] Set ${location.tensions.length} tension(s) at ${location.name}`);
        }
...
async function executeGetScene(plotId) {
    ...
    const tensions = (currentLoc.tensions || []).map(t => ({
        description: t.description,
        involvedNpcs: t.involvedNpcs || [],
        severity: t.severity || 'simmering'
    }));
